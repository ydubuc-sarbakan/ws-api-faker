    ...
    handled(requestEnvelope: AppEnvelope, socket: WebSocket): boolean {
        switch (requestEnvelope.action) {
            case PlayerFinishedRaceClientRequest.ACTION: {
                this.arcadeController.handlePlayerFinishedRaceClientRequest(requestEnvelope.openAs(PlayerFinishedRaceClientRequest), socket);
                return true;
            }
            case PlayerFinishedCupClientRequest.ACTION: {
                this.arcadeController.handlePlayerFinishedCupClientRequest(requestEnvelope.openAs(PlayerFinishedCupClientRequest), socket);
                return true;
            }
            default:
                return false;
